...
describe('GET /api/settings', () => {
  ...
  it('returns managed marker without leaking creds', async () => {
    ...
    expect(res.body.managedApiKey).toBeUndefined();
  });
...
describe('POST /api/settings', () => {
  ...
  it('accepts managed mode without explicit creds', async () => {
    const res = await request(app).post('/api/settings').send({ providerMode: 'managed' });
    expect(res.status).toBe(200);
    expect(res.body.success).toBe(true);
  });
});
