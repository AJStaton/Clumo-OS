...
All AI functionality runs in **BYOK (bring your own key)** mode: the user provides
their own OpenAI or Azure OpenAI credentials. There is no managed/shared key and no
middleman — a user's keys, transcripts, and prompts go only to their own AI provider.

The provider abstraction in `server/ai-provider.js` is BYOK-only. Any new AI provider
integration must implement the same interface as `AzureOpenAIProvider` and `OpenAIProvider`.
