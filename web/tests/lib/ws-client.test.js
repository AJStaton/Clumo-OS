...
describe('ws-client.createWsClient', () => {
  ...
  it('sendAudio sends JSON payload with default customer channel when open', () => {
    const client = createWsClient(() => {});
    const ws = MockWebSocket.instances[0];
    ws._open();
    client.sendAudio('base64audio');
    expect(JSON.parse(ws.sent[0])).toEqual({ type: 'audio', data: 'base64audio', channel: 'customer' });
  });

  it('sendAudio tags the channel when one is given', () => {
    const client = createWsClient(() => {});
    const ws = MockWebSocket.instances[0];
    ws._open();
    client.sendAudio('mic-bytes', 'you');
    expect(JSON.parse(ws.sent[0])).toEqual({ type: 'audio', data: 'mic-bytes', channel: 'you' });
  });
